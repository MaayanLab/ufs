...
def test_ls(m):
    ...
    assert sorted(m.ls("/dir", False)) == ["/dir/afile", "/dir/dir1"]
    assert sorted(m.ls("/dir", True), key=lambda i: i['name'])[0]["type"] == "file"
    assert sorted(m.ls("/dir", True), key=lambda i: i['name'])[1]["type"] == "directory"
