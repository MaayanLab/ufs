...
def test_mountable(ufs: UFS, fuse):
  ...
  with mount(Logger(ufs), fuse=fuse) as mount_dir:
    ...
    # native cp should work
    import subprocess
    assert subprocess.check_call(['cat', str(path/'B')]) == 0
    assert subprocess.check_call(['cp', str(path/'B'), str(path/'A')]) == 0
